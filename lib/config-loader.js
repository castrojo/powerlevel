...
export function validateConfig(config) {
  ...
  if (config.projectBoard) {
    const { enabled, projectNumber, autoCreateFields, fieldMapping } = config.projectBoard;

    if (enabled !== undefined && typeof enabled !== 'boolean') {
      throw new Error('projectBoard.enabled must be a boolean');
    }

    if (projectNumber !== undefined && projectNumber !== null && typeof projectNumber !== 'number') {
      throw new Error('projectBoard.projectNumber must be a number or null');
    }

    if (autoCreateFields !== undefined && typeof autoCreateFields !== 'boolean') {
      throw new Error('projectBoard.autoCreateFields must be a boolean');
    }

    if (fieldMapping !== undefined && (typeof fieldMapping !== 'object' || Array.isArray(fieldMapping))) {
      throw new Error('projectBoard.fieldMapping must be an object');
    }
  }
}
...
export function getProjectBoardConfigFromEnv() {
  ...
  if (process.env.GITHUB_TRACKER_PROJECT_NUMBER) {
    config.projectNumber = parseInt(process.env.GITHUB_TRACKER_PROJECT_NUMBER, 10);
  }
  
  if (process.env.GITHUB_TRACKER_PROJECT_AUTO_CREATE_FIELDS !== undefined) {
    config.autoCreateFields = process.env.GITHUB_TRACKER_PROJECT_AUTO_CREATE_FIELDS === 'true';
  }
