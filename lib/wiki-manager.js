...
export function wikiExists(owner, repo, client = null) {
  ...
  try {
    ...
    logDebug(client, `Wiki check failed for ${owner}/${repo}: ${error.message}`);
    return false;
  }
}
...
export function cloneWiki(owner, repo, client = null) {
  ...
  try {
    if (existsSync(join(wikiDir, '.git'))) {
      ...
      logInfo(client, `Updating wiki cache for ${owner}/${repo}...`);
      ...
      logInfo(client, `Cloning wiki for ${owner}/${repo}...`);
...
export function syncSkillsToWiki(skillsDir, wikiDir, client = null) {
  ...
  for (const entry of entries) {
    ...
    if (!existsSync(skillFile)) {
      logInfo(client, `Skipping ${skillName}: no SKILL.md found`);
      continue;
    }
    ...
    logInfo(client, `Synced ${skillName} -> ${wikiPageName}`);
    syncedCount++;
  }
...
export function commitAndPushWiki(wikiDir, message, client = null) {
  ...
  try {
    ...
    if (!status) {
      logInfo(client, 'No changes to commit');
      return false;
    }
    ...
    logInfo(client, `Successfully committed and pushed: ${message}`);
    return true;
  } catch (error) {
