 ...
 */
export function detectProjectBoard(owner, client = null) {
  try {
    ...
    logError(client, `Failed to detect project board: ${error.message}`);
    return null;
  }
}
 ...
 * @returns {Object|null} Project board object with id, number, title, owner, url, and detected_at, or null if not found
 */
export function getProjectBoard(owner, projectNumber, client = null) {
  try {
    const projectsJson = execGh(`project list --owner ${owner} --format json`);
    const projects = JSON.parse(projectsJson);
    ...
    logError(client, `Failed to get project board: ${error.message}`);
    return null;
  }
}
