...
export function hasRemote(remoteName, cwd, client = null) {
  ...
  try {
    ...
    logDebug(client, `Failed to check remote '${remoteName}': ${error.message}`);
    return false;
  }
}
...
export function getRemoteUrl(remoteName, cwd, client = null) {
  ...
  try {
    ...
    logDebug(client, `Failed to get URL for remote '${remoteName}': ${error.message}`);
    return null;
  }
}
...
export function addRemote(remoteName, url, cwd, client = null) {
  ...
  try {
    ...
    logInfo(client, `Successfully added remote '${remoteName}': ${url}`);
  } catch (error) {
...
export function fetchRemote(remoteName, cwd, client = null) {
  ...
  try {
    ...
    logInfo(client, `Successfully fetched from remote '${remoteName}'`);
  } catch (error) {
...
export function detectRepoFromRemote(remoteName, cwd, client = null) {
  ...
  if (!remoteUrl) {
    logError(client, `Remote '${remoteName}' not found in repository`);
    return null;
  }
  ...
  if (!repoInfo) {
    logError(client, `Unable to parse GitHub URL from remote '${remoteName}': ${remoteUrl}`);
    return null;
  }
