 ...
 */
export function hasRemote(remoteName, cwd) {
  ...
  try {
    ...
    console.debug(`Failed to check remote '${remoteName}': ${error.message}`);
    return false;
  }
}
...
export function getRemoteUrl(remoteName, cwd) {
  ...
  try {
    ...
    console.debug(`Failed to get URL for remote '${remoteName}': ${error.message}`);
    return null;
  }
}
 ...
 */
export function addRemote(remoteName, url, cwd) {
  if (!remoteName || typeof remoteName !== 'string') {
    throw new Error('remoteName must be a non-empty string');
  }
  ...
  try {
    ...
    console.log(`Successfully added remote '${remoteName}': ${url}`);
  } catch (error) {
 ...
 */
export function fetchRemote(remoteName, cwd) {
  if (!remoteName || typeof remoteName !== 'string') {
    throw new Error('remoteName must be a non-empty string');
  }
  ...
  try {
    ...
    console.log(`Successfully fetched from remote '${remoteName}'`);
  } catch (error) {
 ...
 */
export function detectRepoFromRemote(remoteName, cwd) {
  if (!remoteName || typeof remoteName !== 'string') {
    throw new Error('remoteName must be a non-empty string');
  }
  ...
  const remoteUrl = getRemoteUrl(remoteName, cwd);
  
  if (!remoteUrl) {
    console.error(`Remote '${remoteName}' not found in repository`);
    return null;
  }
  ...
  if (!repoInfo) {
    console.error(`Unable to parse GitHub URL from remote '${remoteName}': ${remoteUrl}`);
    return null;
  }
