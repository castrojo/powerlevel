...
export const LABELS = {
  ...
  // Project labels (dynamically generated with project name)
  // Format: 'project/name' - created as needed
 ...
 */
export async function ensureLabelsExist(repo) {
  ...
  console.log(`Creating ${labelsToCreate.length} missing labels...`);
 ...
 * @param {string} projectName - (Optional) Project name for project label
 * @returns {string[]} Array of label names
 */
export function getEpicLabels(priority = 'p2', projectName = null) {
  const labels = [
    'type/epic',
    `priority/${priority}`,
    'status/planning'
 ...
 * @returns {string[]} Array of label names
 */
export function getTaskLabels(epicNumber, priority = 'p2', projectName = null) {
  const labels = [
    'type/task',
    `epic/${epicNumber}`,
    `task/${priority}`,
 ...
 */
export function ensureProjectLabel(repo, projectName) {
  const labelName = `project/${projectName}`;
  
  try {
    // Try to create the label (will fail silently if it exists)
    execGh(`label create "${labelName}" --repo ${repo} --color 0052cc --description "Issues for project ${projectName}" 2>/dev/null || true`);
  } catch (error) {
    // Label might already exist, which is fine
  }
