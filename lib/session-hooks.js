import { loadCache, saveCache, addJourneyEntry } from './cache-manager.js';
...
export function registerSessionHooks(session, owner, repo, cwd, client = null) {
  if (!session || !session.on) {
    logWarn(client, 'Session does not support event hooks');
    return;
  }
  ...
  session.on('assistant.message', async (message) => {
    ...
    if (skillInfo) {
      logDebug(client, `Detected skill: ${skillInfo.skill}`);
  ...
  session.on('file.created', async (file) => {
    if (file.path && file.path.includes('docs/plans/')) {
      logDebug(client, `Plan file created: ${file.path}`);
      await handlePlanCreation(file.path, owner, repo, cwd, client);
    }
  });

  logInfo(client, '✓ Registered session event hooks for Superpowers integration');
}
...
async function handleExecutingPlans(message, cache, owner, repo, cwd, client = null) {
  ...
  if (planFile) {
    ...
    if (epic) {
      logInfo(client, `Linked executing-plans to epic #${epic.number}`);
      
      // Track event in epic journey
      addJourneyEntry(cache, epic.number, {
...
async function handleFinishingBranch(message, cache, owner, repo, cwd, client = null) {
  ...
  if (activeEpic) {
    logInfo(client, `Linked finishing-branch to epic #${activeEpic.number}`);
    
    // Track event in epic journey
    addJourneyEntry(cache, activeEpic.number, {
      timestamp: new Date().toISOString(),
      event: 'skill_invocation',
...
async function handleSubagentDevelopment(message, cache, owner, repo, cwd, client = null) {
  ...
  if (planFile) {
    ...
    if (epic) {
      logInfo(client, `Linked subagent-driven-development to epic #${epic.number}`);
...
async function handlePreparingUpstreamPR(message, cache, owner, repo, cwd, client = null) {
  ...
  if (activeEpic) {
    logInfo(client, `Linked preparing-upstream-pr to epic #${activeEpic.number}`);
...
async function handlePlanCreation(planPath, owner, repo, cwd, client = null) {
  logDebug(client, `Plan created: ${planPath}`);
...
function findActiveEpic(cache, cwd) {
  ...
  const inProgressEpics = cache.epics.filter(epic =>
    epic.state !== 'closed' &&
  ...
  if (inProgressEpics.length === 0) {
    ...
    const openEpics = cache.epics.filter(epic => epic.state !== 'closed');
