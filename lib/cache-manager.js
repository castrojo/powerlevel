 ...
 */
export function loadCache(owner, repo, client = null) {
    ...
    if (client) {
      client.app.log({
        body: {
          service: 'powerlevel',
          level: 'error',
          message: `Error loading cache: ${error.message}`
        }
      });
    }
    return {
 ...
 * @param {Object} client - OpenCode SDK client (optional)
 */
export function saveCache(owner, repo, cache, client = null) {
  const cachePath = getCachePath(owner, repo);
    ...
    if (client) {
      client.app.log({
        body: {
          service: 'powerlevel',
          level: 'error',
          message: `Error saving cache: ${error.message}`
        }
      });
    }
    throw error;
...
/**
 * Gets an epic from cache by number
 * @param {Object} cache - Cache object
 * @param {number} epicNumber - Epic issue number
...
/**
 * Adds a journey entry to an epic
 * @param {Object} cache - Cache object
 * @param {number} epicNumber - Epic issue number
 * @param {Object} entry - Journey entry with timestamp, event, message
 * @returns {Object} Updated cache
 */
