 ...
 */
export function fetchRepoMetadata(owner, repo, client = null) {
  try {
    ...
    logError(client, `Failed to fetch repo metadata: ${error.message}`);
    return null;
  }
}
 ...
 */
export async function promptForProjectName(suggestedName, powerlevelPath, client = null) {
  return new Promise((resolve) => {
    ...
    const askForName = () => {
      rl.question(`\nEnter a different project name: `, (answer) => {
        ...
        if (!newName) {
          logError(client, 'Error: Project name cannot be empty');
          askForName();
          return;
        }
        
        if (projectExists(powerlevelPath, newName)) {
          logError(client, `Error: Project '${newName}' already exists`);
          askForName();
          return;
        }
