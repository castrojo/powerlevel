    ...
    if (client) {
      client.app.log({
        body: {
          service: 'powerlevel',
          level: 'info',
          message: `Fetched ${issues.length} open issues from ${externalRepo}`
        }
      });
    }
    ...
    if (client) {
      client.app.log({
        body: {
          service: 'powerlevel',
          level: 'error',
          message: `Error fetching issues from ${externalRepo}: ${error.message}`
        }
      });
    }
    return [];
    ...
    if (client) {
      client.app.log({
        body: {
          service: 'powerlevel',
          level: 'info',
          message: `✓ Synced Epic #${epicNumber}: ${issues.length} items tracked`
        }
      });
    }
...
/**
 * Extracts external repo from epic body
 * @param {string} body - Epic body text
 * @returns {string|null} External repo in owner/repo format or null
 */
export function extractExternalRepo(body) {
  if (!body) return null;
  
  // Match pattern: [owner/repo](https://github.com/owner/repo)
  const match = body.match(/\[([^/]+\/[^\]]+)\]\(https:\/\/github\.com\/[^)]+\)/);
  if (match) return match[1];
  
  // Fallback: Match raw URL pattern
