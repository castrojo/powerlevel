 ...
 * @returns {void}
 */
export function addJourneyEntry(epicNumber, entry, cwd, client = null) {
  ...
  try {
    ...
    logInfo(client, `✅ Added journey entry to epic #${epicNumber}`);
  } catch (error) {
    logError(client, `Error adding journey entry to epic #${epicNumber}: ${error.message}`);
    throw error;
  }
}
 ...
 * @returns {void}
 */
export function syncEpicToGitHub(epicNumber, cwd, client = null) {
  validateEpicNumber(epicNumber);
  ...
  try {
    ...
    if (!config.tracking.autoUpdateEpics) {
      logWarn(client, `⚠️  Epic auto-updates disabled in config`);
      return;
    }
    ...
    if (!epic.dirty) {
      logInfo(client, `ℹ️  Epic #${epicNumber} is already in sync`);
      return;
    }
    ...
    try {
      ...
      logInfo(client, `✅ Synced epic #${epicNumber} to GitHub`);
    } catch (ghError) {
    ...
    if (error.message.includes('rate limit') || error.message.includes('network')) {
      ...
      logError(client, `⚠️  Network error syncing epic #${epicNumber}: ${error.message}\nEpic remains marked as dirty for retry`);
    } else {
 ...
 * @returns {void}
 */
export function recordTaskCompletion(epicNumber, taskNumber, taskTitle, agentInfo, cwd, client = null) {
  validateEpicNumber(epicNumber);
  ...
  addJourneyEntry(epicNumber, entry, cwd, client);
  ...
  try {
    ...
    if (config.tracking.commentOnProgress) {
      ...
      logInfo(client, `✅ Added completion comment to epic #${epicNumber}`);
    }
  } catch (error) {
    // Non-critical: log but don't throw
    logWarn(client, `Warning: Could not add comment to epic #${epicNumber}: ${error.message}`);
  }
}
